...
scene.clearColor = new BABYLON.Color4(0.012, 0.006, 0.035, 1);

const cam = new BABYLON.FreeCamera('cam', new BABYLON.Vector3(0, 4.6, -11), scene);
cam.rotation.x = 0.10;
cam.fov = 0.95;

const hemi = new BABYLON.HemisphericLight('h', new BABYLON.Vector3(0, 1, 0), scene);
hemi.intensity = 0.22;
hemi.diffuse = new BABYLON.Color3(0.45, 0.35, 0.75);
hemi.groundColor = new BABYLON.Color3(0.05, 0.02, 0.1);

// ------------------------------------------------------------ textures ----
function windowTexture() {
  const dt = new BABYLON.DynamicTexture('win', { width: 256, height: 512 }, scene, true);
...
function sunTexture() {
  const dt = new BABYLON.DynamicTexture('sun', { width: 512, height: 512 }, scene, true);
  const x = dt.getContext();
...
function ridgeTexture(seed, dark) {
  const dt = new BABYLON.DynamicTexture('ridge' + seed, { width: 1024, height: 256 }, scene, true);
  const x = dt.getContext();
...
function skyTexture() {
  const dt = new BABYLON.DynamicTexture('sky', { width: 64, height: 512 }, scene, true);
  const x = dt.getContext();
...
function palmTexture() {
  const dt = new BABYLON.DynamicTexture('palm', { width: 256, height: 512 }, scene, true);
  const x = dt.getContext();
...
// -------------------------------------------------------------- backdrop ----
const skyP = BABYLON.MeshBuilder.CreatePlane('sky', { width: 2600, height: 700 }, scene);
...
const sun = BABYLON.MeshBuilder.CreatePlane('sun', { size: 340 }, scene);
sun.position.set(0, 120, 860);
const sunM = new BABYLON.StandardMaterial('sunm', scene);
...
for (const [seed, z, y, col] of [[1.7, 820, 55, '#241040'], [4.2, 780, 40, '#160a2b']]) {
  const r = BABYLON.MeshBuilder.CreatePlane('r' + z, { width: 2400, height: 170 }, scene);
  r.position.set(0, y, z);
  const m = new BABYLON.StandardMaterial('rm' + z, scene);
...
const ground = BABYLON.MeshBuilder.CreateGround('g', { width: 260, height: VIEW + 80 }, scene);
ground.position.z = VIEW / 2 - 40;
const gm = new BABYLON.PBRMaterial('gm', scene);
gm.albedoColor = new BABYLON.Color3(0.012, 0.012, 0.022);
gm.metallic = 0.8; gm.roughness = 0.3;
const mirror = new BABYLON.MirrorTexture('mir', 1024, scene, true);
...
for (let z = 0; z < VIEW; z += 46) {
  for (const sd of [-1, 1]) {
    const p = BABYLON.MeshBuilder.CreateBox('e', { width: 0.22, height: 0.06, depth: 46 }, scene);
    p.position.set(sd * (ROADW / 2), 0.03, z + 23);
    p.material = cyan; mover(p); mirror.renderList.push(p);
  }
  const c2 = BABYLON.MeshBuilder.CreateBox('cl', { width: 0.16, height: 0.06, depth: 46 }, scene);
...
for (let z = 0; z < VIEW; z += 14) {
  for (const sd of [-1, 1]) {
    const d = BABYLON.MeshBuilder.CreateBox('d', { width: 0.24, height: 0.055, depth: 3.4 }, scene);
...
for (let z = 0; z < VIEW; z += 38) {
  for (const sd of [-1, 1]) {
    const pl = BABYLON.MeshBuilder.CreatePlane('p', { width: 9, height: 18 }, scene);
    ...
    m.emissiveColor = new BABYLON.Color3(0.35, 0.9, 0.85);
...
function buildCar(paintCol, taillit) {
  ...
  const paint = new BABYLON.PBRMaterial('paint', scene);
  paint.albedoColor = paintCol;
  paint.metallic = 0.85; paint.roughness = 0.24;
  paint.reflectionTexture = mirror;
  const blk = new BABYLON.PBRMaterial('blk', scene);
  blk.albedoColor = new BABYLON.Color3(0.02, 0.02, 0.025);
  blk.metallic = 0.3; blk.roughness = 0.6;
  const part = (w, h, d, x, y, z, mat, rx) => {
    const b = BABYLON.MeshBuilder.CreateBox('p', { width: w, height: h, depth: d }, scene);
  ...
  const wm = new BABYLON.PBRMaterial('wm', scene);
  wm.albedoColor = new BABYLON.Color3(0.03, 0.03, 0.03); wm.roughness = 0.9;
  for (const [wx, wz] of [[-0.95, 1.45], [0.95, 1.45], [-0.95, -1.35], [0.95, -1.35]]) {
    const w = BABYLON.MeshBuilder.CreateCylinder('w', { diameter: 0.76, height: 0.3, tessellation: 14 }, scene);
    w.rotation.z = Math.PI / 2; w.position.set(wx, 0.38, wz); w.parent = root; w.material = wm;
    mirror.renderList.push(w);
  }
  return root;
}
const car = buildCar(new BABYLON.Color3(0.55, 0.02, 0.03), true);
...
for (let i = 0; i < 6; i++) {
  ...
  const t = buildCar(new BABYLON.Color3(0.06 + Math.random() * 0.4, 0.05 + Math.random() * 0.3,
...
// ---------------------------------------------------------------- post ----
new BABYLON.GlowLayer('glow', scene, { intensity: 0.55 });
